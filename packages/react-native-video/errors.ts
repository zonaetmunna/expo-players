...
export function describeError(raw: OnVideoErrorData | unknown): FriendlyError {
  ...
  if (
    /parserexception|malformedmanifest|manifest.*error|invalid manifest|unable to parse/.test(text)
  ) {
    return {
  ...
  if (
    /unsupported.*format|unrecognized.*format|cannot be opened|not a valid|cannot decode/.test(text)
  ) {
    return {
  ...
  if (
    /exoplaybackexception.*source|source\s?error|io_bad_http_status|behindlivewindow/.test(text)
  ) {
    return {
